...
public class DishServiceImpl extends ServiceImpl<DishMapper, Dish> implements DishService {
    ...
    public DishDto getByIdWithFlavor(Long id) {
        ...
        // 两个表信息关联 DishFlavor::getDishId 和 dish中的id对应
    ...
    /*@Override
    public boolean updateBatchByIds(List<Dish> dishList) {
        boolean flag=false;
        for (Dish dish : dishList) {
            log.info("id@@@@@@@@@@@@@@@@@@:{}",dish.getId().toString());
            int updateById = dishMapper.updateById(dish);
            if (updateById==0){
                return flag;
            }
        }
     ...
     */
    /*@Override
    public void removeBatchWithFlavor(Long[] ids) {
        //先删除 菜品口味表
        //构建条件器
        LambdaQueryWrapper<DishFlavor> queryWrapper=new LambdaQueryWrapper<>();
